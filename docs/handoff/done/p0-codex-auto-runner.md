...
## Acceptance Criteria
- [x] `scripts/codex-runner.sh` scans backlog and processes tasks in priority order
- [x] Tasks move from `backlog/` to `done/` on success
- [x] Tasks move back to `backlog/` on failure with error notes appended
- [x] Lock file prevents concurrent runs
- [x] All activity logged with timestamps to `scripts/codex-runner.log`
- [x] `scripts/setup-scheduler.ps1` registers a Windows Task Scheduler job to run every 10 minutes
- [x] `.gitignore` updated to exclude `.codex-runner.lock` and `codex-runner.log`
- [x] Script tested: place a dummy task in backlog, run the script, verify it gets processed
...
- The runner handles priority ordering via filename sort, lock-file based single-instance execution, backlog/in-progress/done moves, retry notes on failure, and `TEST_MODE` support for safe smoke testing.
- Verification: `scripts/setup-scheduler.ps1` parsed successfully in PowerShell, `codex-runner.sh` passed `bash -n`, and a real Git Bash smoke test with `CODEX_RUNNER_TEST_MODE=1` moved a dummy task from temp backlog to temp done while writing the expected log file.
- Decision made: the runner supports override environment variables for the backlog, in-progress, done, lock, log, and prompt paths so it can be smoke-tested safely without touching the real handoff queue.
